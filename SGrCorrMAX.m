function [Gengmax,Logmax,ENSgmax,Genmax,Loadmax,ENSmax,OverChargemax,SolarStorageGmax,SSGgmax]=SGrCorrMAX(lf,BUS,BRANCH,Title,Genmaxi,Loadmaxi,ENSmaxi,OverChargemaxi,UCf,t,lmax,savefilename,Ymatriz,Schf,lnpgen)
...
Type=Title.case;
savingtxt=strcat(Type,'Correction',savefilename,'MAX',num2str(t),'.txt');
savingdir=strcat(savefilename,'Correction');
cd(pwd);
...
for n=1:nbus
    if Pgmax(n,1)>Schf(t,n)
        SolarStorageGmax(n)=Schf(t,n);
    else
        SolarStorageGmax(n)=Pgmax(n);
    end
    SSGgmax=SSGgmax+SolarStorageGmax(n);
end

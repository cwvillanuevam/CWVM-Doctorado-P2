function [GenNol,GenNmin,GenNmax,LoadNol,LoadNmin,LoadNmax,ENSNol,ENSNmin,ENSNmax,OverChargeNol,OverChargeNmin,OverChargeNmax,savefileNname,NSPcap,Schf,BSScap,PVSScost]=SolarnewPSevalu(lftype,Ns,T,V,savefilename,MCSmin,MCSq1,MCSmean,MCSq3,MCSmax,lf,UCf,lmin,lmax,TScf,Ymatriz,BUS,BRANCH,Title,NPgen,lnpgen,tpgen,ENSgmin,ENSgmax,minmaxind)
...
Type=Title.case;
savingtxt=strcat(Type,'SolarNewPSvalidacion.txt');
savingdir=strcat(Type,'SolarNewPSvalidaciondir');
version=1;
while exist(savingdir,'file')==7 || exist(savingdir,'file')==2 
    savingtxt=strcat(Type,'SolarNewPSvalidacion','v',num2str(version),'.txt');
    savingdir=strcat(Type,'SolarNewPSvalidaciondir','v',num2str(version));
    version=version+1;
end
mkdir(savingdir)
cd(pwd);

[NSPcap,Schf,BSScap,PVSScost,PVSScap]=GetInf(Sch,ntime,nbus,BUS,ENSgmin,ENSgmax,minmaxind,savingdir);

for n=1:nbus
    if PVSScap(n)==0
    else
    BUS.TipoBus(n)=2;
%     BUS.PGenBus(n,1)=BUS.PGenBus(n,1)+PVSScap(n,1);
    end
    if BUS.TipoBus(n)==0
        nbusPQ(1,PQindex)=n;
        PQindex=PQindex+1;
...
SolarStorageGol=zeros(ntime,nbus);
SSGgol=zeros(ntime,1);
SolarStorageGmin=zeros(ntime,nbus);
SSGgmin=zeros(ntime,1);
SolarStorageGmax=zeros(ntime,nbus);
SSGgmax=zeros(ntime,1);
...
for n=1:nbus
    if Pgol(n,1)>Schf(t,n)
        SolarStorageGol(t,n)=Schf(t,n);
    else
        SolarStorageGol(t,n)=Pgol(n,1);
    end
    SSGgol(t,1)=SSGgol(t,1)+SolarStorageGol(t,n);
end
...
fprintf('\nt:%d',t);
while opfolNite<=2
[GenNgoltemp,LoadNgoltemp,ENSNgoltemp,GenNol,LoadNol,ENSNol,OverChargeNol,savingtxttemp,SolarStorageGoltemp,SSGgoltemp]=SGrCorrOL(GenNol,LoadNol,ENSNol,OverChargeNol,lf,BUS,BRANCH,Title,UCf,t,savingdir,Ymatriz,1,Schf,lnpgen);
for n=1:nbus
    Gengol(t,n)=GenNgoltemp(n);
    Logol(t,n)=LoadNgoltemp(n);
    ENSgol(t,n)=ENSNgoltemp(n);
SolarStorageGol(t,n)=SolarStorageGoltemp(n);
end
SSGgol(t,1)=SSGgoltemp;
...
fprintf(fresNPS,'bus:\tSolar Cap.:\tStorage Cap.:');
for t=1:ntime
    fprintf(fresNPS,'\t t:%2d',t);
end
for n=1:nbus
    fprintf(fresNPS,'\n%2d\t%6.2fkW\t%6.2fkWh',n,NSPcap(n),BSScap(n));
    for t=1:ntime
        fprintf(fresNPS,'\t %6.2f',SolarStorageGol(t,n));
    end
end
fprintf(fresNPS,'\nGlobal Solar Storage System Supply:');
for t=1:ntime
    fprintf(fresNPS,'\t%6.2f',SSGgol(t,1));
end
...
for n=1:nbus
    if Pgmin(n,1)>Schf(t,n)
        SolarStorageGmin(t,n)=Schf(t,n);
    else
        SolarStorageGmin(t,n)=Pgmin(n);
    end
    SSGgmin(t,1)=SSGgmin(t,1)+SolarStorageGmin(t,n);
end
...
fprintf('\nt:%d',t);
while opfNminite<=2
[GenNgmintemp,LoadNgmintemp,ENSNgmintemp,GenNmin,LoadNmin,ENSNmin,OverChargeNmin,SolarStorageGmintemp,SSGgmintemp]=SGrCorrMIN(lf,BUS,BRANCH,Title,GenNmin,LoadNmin,ENSNmin,OverChargeNmin,UCf,t,lmin,savingdir,Ymatriz,Schf,lnpgen);
for n=1:nbus
    Gengmin(t,n)=GenNgmintemp(n);
    Logmin(t,n)=LoadNgmintemp(n);
    ENSgmin(t,n)=ENSNgmintemp(n);
SolarStorageGmin(t,n)=SolarStorageGmintemp(n);
end
SSGgmin(t,1)=SSGgmintemp;
...
for n=1:nbus
    fprintf(fresNPS,'\n%2d\t%6.2fkW\t%6.2fkWh',n,NSPcap(n),BSScap(n));
    for t=1:ntime
        fprintf(fresNPS,'\t %6.2f',SolarStorageGmin(t,n));
    end
end
fprintf(fresNPS,'\nGlobal Solar Storage System Supply:');
for t=1:ntime
    fprintf(fresNPS,'\t%6.2f',SSGgmin(t,1));
end
...
SSGgmax(t,1)=0;
for n=1:nbus
    if Pgmax(n,1)>Schf(t,n)
        SolarStorageGmax(t,n)=Schf(t,n);
    else
        SolarStorageGmax(t,n)=Pgmax(n);
    end
    SSGgmax(t,1)=SSGgmax(t,1)+SolarStorageGmax(t,n);
end
...
fprintf('\nt:%d',t);
while opfmaxNite<=2
[GenNgmaxtemp,LoadNgmaxtemp,ENSNgmaxtemp,GenNmax,LoadNmax,ENSNmax,OverChargeNmax,SolarStorageGmaxtemp,SSGgmaxtemp]=SGrCorrMAX(lf,BUS,BRANCH,Title,GenNmax,LoadNmax,ENSNmax,OverChargeNmax,UCf,t,lmax,savingdir,Ymatriz,Schf,lnpgen);
for n=1:nbus
    Gengmax(t,n)=GenNgmaxtemp(n);
    Logmax(t,n)=LoadNgmaxtemp(n);
    ENSgmax(t,n)=ENSNgmaxtemp(n);
    SolarStorageGmax(t,n)=SolarStorageGmaxtemp(n);
end
SSGgmax(t,1)=SSGgmaxtemp;
...
for n=1:nbus
    fprintf(fresNPS,'\n%2d\t%6.2fkW\t%6.2fkWh',n,NSPcap(n),BSScap(n));
    for t=1:ntime
        fprintf(fresNPS,'\t %6.2f',SolarStorageGmax(t,n));
    end
end
fprintf(fresNPS,'\nGlobal Solar Storage System Supply:');
for t=1:ntime
    fprintf(fresNPS,'\t%6.2f',SSGgmax(t,1));
end
...
lgd1=legend({'Generation','Load','Energy Not Served','Overcharge','Solar-Storage System'},'location','east');
lgd1.FontSize=8;
xlabel('time 1/2 h');
ylabel('MW');
F10txt='OlNewPS.jpg';
...
lgd2=legend({'Generation','Load','Energy Not Served','Overcharge','Solar-Storage System'},'location','east');
lgd2.FontSize=8;
xlabel('time 1/2 h');
ylabel('MW');
F11txt='MinNewPS.jpg';
...
lgd3=legend({'Generation','Load','Energy Not Served','Overcharge','Solar-Storage System'},'location','east');
lgd3.FontSize=8;
xlabel('time 1/2 h');
ylabel('MW');
F12txt='MaxNewPS.jpg';
...
if strcmp(GCfix,'Col')
[GenNgoltemp,LoadNgoltemp,ENSNgoltemp,GenNol,LoadNol,ENSNol,OverChargeNol,savingtxttemp,SolarStorageGoltemp,SSGgoltemp]=SGrCorrOL(GenNol,LoadNol,ENSNol,OverChargeNol,lf,BUS,BRANCH,Title,UCf,tfix,savingdir,Ymatriz,1,Schf,lnpgen);
for n=1:nbus
    Gengol(tfix,n)=GenNgoltemp(n);
    Logol(tfix,n)=LoadNgoltemp(n);
    ENSgol(tfix,n)=ENSNgoltemp(n);
SolarStorageGol(tfix,n)=SolarStorageGoltemp(n);
end
SSGgol(tfix,1)=SSGgoltemp;
elseif strcmp(GCfix,'Cmin')
[GenNgmintemp,LoadNgmintemp,ENSNgmintemp,GenNmin,LoadNmin,ENSNmin,OverChargeNmin,SolarStorageGmintemp,SSGgmintemp]=SGrCorrMIN(lf,BUS,BRANCH,Title,GenNmin,LoadNmin,ENSNmin,OverChargeNmin,UCf,tfix,lmin,savingdir,Ymatriz,Schf,lnpgen);
for n=1:nbus
    Gengmin(tfix,n)=GenNgmintemp(n);
    Logmin(tfix,n)=LoadNgmintemp(n);
    ENSgmin(tfix,n)=ENSNgmintemp(n);
SolarStorageGmin(tfix,n)=SolarStorageGmintemp(n);
end
SSGgmin(tfix,1)=SSGgmintemp;
elseif strcmp(GCfix,'Cmax')
[GenNgmaxtemp,LoadNgmaxtemp,ENSNgmaxtemp,GenNmax,LoadNmax,ENSNmax,OverChargeNmax,SolarStorageGmaxtemp,SSGgmaxtemp]=SGrCorrMAX(lf,BUS,BRANCH,Title,GenNmax,LoadNmax,ENSNmax,OverChargeNmax,UCf,tfix,lmax,savingdir,Ymatriz,Schf,lnpgen);

for n=1:nbus
    Gengmax(tfix,n)=GenNgmaxtemp(n);
    Logmax(tfix,n)=LoadNgmaxtemp(n);
    ENSgmax(tfix,n)=ENSNgmaxtemp(n);
SolarStorageGmax(tfix,n)=SolarStorageGmaxtemp(n);
end
SSGgmax(tfix,1)=SSGgmaxtemp;
else
